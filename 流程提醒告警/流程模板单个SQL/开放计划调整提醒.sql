with tmpl as (
    ...
    where t.TMPL_NAME in ('HXLC_开放计划调整')
),
open_plan_node_cfg as (
    ...
    union all select '草稿', 'PRE_DEPT_APPR', '流程未过提交部门审批节点'
...
current_todo as (
    ...
    left join open_plan_node_cfg nc
...
select distinct
    ...
    concat(
        ...
        '\n',
        '业务生效日：',
        date_format(ti.OPEN_DATE, '%Y-%m-%d'),
        '\n',
