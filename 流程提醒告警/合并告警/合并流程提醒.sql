-- 合并流程提醒告警 SQL
-- 功能: 整合[产品申报登记]、[产品发行审批]、[产品发行前变更]、[产品暂停发行]、[产品发行登记]、[产品定价管理]、[增发份额]、[销售相关信息变更]、[开放计划调整]、[分红方案管理]的流程监控逻辑
-- 
-- 逻辑分支说明:
-- A. 分支逻辑 A: 产品申报登记 (Sb) 
...
target_sales_change as (
    select
        ...
        case
            when ui.SCENE_CODE = 'WAIT_SUBMIT' then 'ALL_PM'
            when ui.SCENE_CODE in ('PRE_DEPT_APPR', 'OTHERS') then 'FLOW_PM'
            else ''
        end as PM_RECEIVER_TYPE,
        1 as SEND_INITIATOR,
        1 as SEND_INV_SALES,
...
target_dividend as (
    select
        ...
        p.PRD_DEPT,
        case
            when ui.TMPL_NAME = 'HXLC_产品分红方案_定期分红' then 'FLOW_PM'
        ...
        case
            when ui.TMPL_NAME = 'HXLC_产品分红方案_定期分红' then 0
            else 1
        end as SEND_INITIATOR,
        1 as SEND_INV_SALES,
        0 as SEND_SB_REGISTER,
        0 as SEND_ISSUE_REGISTER,
        case
...
recipient_inv_sales_flow as (
    ...
    where ba.SEND_INV_SALES = 1
      and ba.PM_RECEIVER_TYPE = 'FLOW_PM'
...
-- 申报登记岗位接收集合（受 SEND_SB_REGISTER 控制）
